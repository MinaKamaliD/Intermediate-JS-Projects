...
function generateText(ev) {
  ev.preventDefault();
  let randomIndex = Math.floor(Math.random() * (dummyText.length + 1));
  ...
  if (
    ...
    result.innerHTML = dummyText[randomIndex];
  } else {
    let slicedArray = dummyText.slice(0, input.value);

    let finalArray = slicedArray
